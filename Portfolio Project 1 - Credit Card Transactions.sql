use cars;
drop table credit_card_transcations;
...
SELECT City , sum(amount) as total_spend , round(sum(amount) * 100/(select sum(amount) from  credit_card_transcations ) , 2) as percentage_contribution
...
select * , month(transaction_date , "month") as month
FROM credit_card_transcations
